...
export const DataCatalogView: React.FC<DataCatalogViewProps> = ({ t, onSelectQuery }) => {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-white">{t.catalog.title}</h1>
        <p className="mt-1 text-sm text-slate-400">{t.catalog.subtitle}</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        {/* Left: Table Navigator Tree */}
        <div className="lg:col-span-1 rounded-xl border border-slate-800 bg-slate-900/60 p-4 backdrop-blur-sm">
          <div className="flex items-center gap-2 mb-3 pb-2 border-b border-slate-800">
            <Database className="h-4 w-4 text-cyan-400" />
            <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Lakekeeper Catalog</span>
          </div>

          <div className="space-y-1">
            <div className="text-xs font-mono text-slate-400 px-2 py-1 font-semibold flex items-center gap-1.5">
              <span>beluga_lake</span>
              <span className="text-[10px] text-slate-500 font-normal">/ default</span>
            </div>
              ...
              {catalogTablesData.map((table) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[10px] rounded bg-slate-800 px-1 font-mono text-slate-400">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-6 backdrop-blur-sm">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-800">
              <div>
                <div className="flex items-center gap-2">
                  <h2 className="text-xl font-bold text-white font-mono">{selectedTable.catalog}.{selectedTable.schema}.{selectedTable.table}</h2>
                  <span className="rounded-full bg-blue-500/10 px-2 py-0.5 text-xs font-medium text-blue-400 border border-blue-500/20 font-mono">
              ...
              <div className="flex items-center gap-4 text-xs">
                <div className="rounded-lg bg-slate-950 p-2.5 border border-slate-800 text-center">
                  <div className="text-[10px] text-slate-400 uppercase font-mono">Snapshots</div>
                  <div className="text-sm font-bold text-white font-mono">{selectedTable.snapshotCount}</div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-5">
              <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-3">{t.catalog.columns}</h3>
                  ...
                  <tbody className="divide-y divide-slate-800/60 font-mono">
                    {selectedTable.columns.map((col) => (
                      <tr key={col.name} className="hover:bg-slate-800/20">
                        ...
                        <td className="py-2.5 px-3 font-sans text-slate-400">{col.comment || '-'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="mt-6 pt-5 border-t border-slate-800">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-semibold uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
                  <Sparkles className="h-3.5 w-3.5 text-cyan-400" /> Trino Query Template
                </span>
                <button
                  onClick={copySql}
              ...
              <pre className="rounded-lg bg-slate-950 p-3 border border-slate-800 text-xs font-mono text-cyan-300/90 overflow-x-auto">
                {sampleSql}
              </pre>
            </div>
          </div>
