  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-white">{t.query.title}</h1>
          <p className="mt-1 text-sm text-slate-400">{t.query.subtitle}</p>
        </div>
        <a
          href="http://trino.local.beluga.internal"
          target="_blank"
          rel="noreferrer"
          className="inline-flex items-center gap-1.5 rounded-lg bg-slate-800 px-3 py-2 text-xs font-medium text-slate-300 hover:text-white hover:bg-slate-700 transition-colors border border-slate-700"
        >
          Trino 코디네이터 UI 열기
      ...
      <div className="flex flex-wrap gap-2 items-center">
        <span className="text-xs text-slate-500 font-medium mr-1">{t.query.presetQueries}:</span>
        {presetQueries.map((preset) => (
          <button
            key={preset.title}
            onClick={() => setSql(preset.sql)}
            className="px-2.5 py-1 rounded-md text-xs font-medium bg-slate-900 border border-slate-800 text-slate-300 hover:border-cyan-500/40 hover:text-cyan-300 transition-colors"
          >
            {preset.title}
          </button>
        ))}
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-900/60 overflow-hidden backdrop-blur-sm shadow-xl">
        <div className="flex items-center justify-between px-4 py-2.5 bg-slate-950/80 border-b border-slate-800 text-xs">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => setSql('')}
              className="flex items-center gap-1 text-slate-400 hover:text-slate-200 px-2 py-1 rounded hover:bg-slate-800 transition-colors"
            >
            ...
            <button
              onClick={handleRun}
              disabled={isRunning}
              className="flex items-center gap-1.5 rounded-md bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-bold px-3 py-1.5 transition-colors disabled:opacity-50"
            >
        ...
        <textarea
          ...
          rows={7}
          className="w-full bg-slate-950/60 p-4 font-mono text-sm text-cyan-200/90 focus:outline-none resize-none selection:bg-cyan-500/30"
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-200">
                {mockResults.map((row, idx) => (
                  <tr key={idx} className="hover:bg-slate-800/20">
                    ...
                    <td className="py-2.5 px-4 text-right">{row.total_orders.toLocaleString()}</td>
                    <td className="py-2.5 px-4 text-right text-emerald-400 font-semibold">{row.revenue}</td>
                  </tr>
                ))}
              </tbody>
            </table>
